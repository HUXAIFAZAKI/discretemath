...
# ---------------------------------------------------------------------------
# Floating Math Particle System (canvas, injected once via JS guard)
# ---------------------------------------------------------------------------
PARTICLES_JS = """
<script>
(function(){
  ...
  canvas.style.cssText = 'position:fixed;top:0;left:0;width:100vw;height:100vh;pointer-events:none;z-index:0;opacity:0.13;';
...
def _make_css(theme: str) -> str:
    ...
    bg          = "#FAFAF8"                    if L else "#060504"
    bg_card     = "rgba(255,252,248,0.98)"     if L else "rgba(13,11,9,0.97)"
    bg_nav_top  = "rgba(252,250,246,0.95)"     if L else "rgba(6,5,4,0.95)"
    sidebar_bg  = ("linear-gradient(165deg,#FEFCF9 0%,#F7F3EE 100%)"
                   if L else
                   "linear-gradient(165deg,#0A0806 0%,#070503 100%)")
    accent      = "#E85520"                    if L else "#FF6B35"
    accent2     = "#FF7A40"                    if L else "#FFA060"
    ag          = "rgba(232,85,32,.11)"        if L else "rgba(255,107,53,.14)"
    am          = "rgba(232,85,32,.06)"        if L else "rgba(255,107,53,.07)"
    text        = "#1C1714"                    if L else "#F2EDE6"
    text_s      = "#6E6560"                    if L else "#9E9890"
    text_d      = "#C8C0B8"                    if L else "#2E2B28"
    bdr         = "rgba(232,85,32,.11)"        if L else "rgba(255,107,53,.10)"
    bdr_b       = "rgba(232,85,32,.38)"        if L else "rgba(255,107,53,.38)"
    scr         = "#CEC8C0"                    if L else "#201E1C"
    lnk_c       = "#706860"
    lnk_ac      = "#E85520"                    if L else "#FF6B35"
    lnk_abg     = "rgba(232,85,32,.07)"        if L else "rgba(255,107,53,.09)"
    h_c         = "#1C1714"                    if L else "#F2EDE6"
    q_bg        = "rgba(250,247,243,.96)"      if L else "rgba(10,8,6,.94)"
    q_bdr       = "rgba(180,170,160,.4)"       if L else "rgba(52,46,40,.8)"
    form_bg     = "rgba(252,249,245,.98)"      if L else "rgba(8,7,5,.97)"
    res_bg      = "rgba(248,244,240,.97)"      if L else "rgba(5,4,3,.97)"
    formula_bg  = "rgba(232,85,32,.05)"        if L else "rgba(255,107,53,.06)"
...
#ds-hamburger:hover {{ border-color: {bdr_b}; box-shadow: 0 2px 12px {ag}; }}
#ds-hamburger .h-bar {{
...
#ds-overlay {{
  ...
  transition: opacity .35s; backdrop-filter: blur(4px); cursor: pointer;
}}
...
#ds-sidenav {{
  ...
  box-shadow: 8px 0 52px rgba(0,0,0,.55);
...
.sn-links a {{
  ...
  transition: all .17s; position: relative; border: 1px solid transparent;
}}
...
a.sn-active .sn-sub {{ color: {accent}77; }}
.sn-week {{
...
@keyframes pulseGlow {{
  0%,100% {{ box-shadow: 0 0 6px {accent}; }}
  50%      {{ box-shadow: 0 0 16px {accent}, 0 0 28px {accent}55; }}
}}
.hero-stats {{
  display: flex; align-items: center; justify-content: center;
...
.card::before {{
  ...
  background: linear-gradient(180deg, transparent, {accent} 40%, {accent} 60%, transparent);
  border-radius: 0 2px 2px 0;
}}
.card:hover {{
  border-color: {bdr_b}; transform: translateY(-2px);
  box-shadow: 0 6px 34px {ag};
}}
@keyframes fadeUp {{
...
.status-badge.partial {{ color: {accent2}; background: rgba(255,160,96,.07); border: 1px solid rgba(255,160,96,.22); }}
...
.stat-box:hover {{ border-color: {bdr_b}; box-shadow: 0 4px 28px {ag}; transform: translateY(-2px); }}
.stat-box-num {{
...
.cta-banner:hover {{ border-color: {bdr_b}; box-shadow: 0 4px 32px {ag}; }}
.cta-banner::after {{
...
.quiz-wrong   {{ border-color: #FF5252 !important; background: rgba(255,82,82,.07) !important; color: #FF5252 !important; }}
...
.team-card:hover {{ box-shadow: 0 4px 22px {ag}; }}
...
.stButton > button {{
  ...
  color: #FFF5EE; border: none; font-weight: 700;
  letter-spacing: .02em; padding: .55rem 1.65rem; border-radius: 9px;
  transition: transform .16s, box-shadow .2s, filter .2s;
  box-shadow: 0 2px 18px {ag}; font-size: .86rem;
  position: relative; overflow: hidden;
}}
.stButton > button::after {{
  content: ''; position: absolute; inset: 0;
  background: linear-gradient(135deg, transparent 40%, rgba(255,255,255,.13) 100%);
  pointer-events: none;
}}
.stButton > button:hover {{ filter: brightness(1.1); box-shadow: 0 4px 28px {ag}; transform: translateY(-1px); }}
...
.divider {{
  height: 1px;
  background: linear-gradient(90deg, transparent, {bdr} 25%, {accent}28 50%, {bdr} 75%, transparent);
  margin: 2.2rem 0;
}}
.sec-tag {{
...
<div style='text-align:center;padding:2rem 0 1.2rem;'>
  ...
  background:linear-gradient(120deg,#FF6B35,#FFA060);-webkit-background-clip:text;
  ...
  color:#706860;margin-top:.5rem;letter-spacing:.1em;'>
    MASTERING THE MATHEMATICS THAT POWERS COMPUTATION
  </div>
  <div style='font-family:"Space Grotesk",sans-serif;font-size:.74rem;color:#706860;margin-top:.9rem;'>
    <span style='color:#9A9490;'>Farhan Haroon</span> \u00b7 FA25-BSAI-0060 &nbsp;|&nbsp;
    <span style='color:#9A9490;'>Huzaifa Zaki</span> \u00b7 FA25-BSAI-0051 &nbsp;|&nbsp;
    <span style='color:#9A9490;'>Bissam ul Haq</span> \u00b7 FA25-BSAI-0076 &nbsp;|&nbsp;
    <span style='color:#9A9490;'>Aaleen</span> \u00b7 FA25-BSAI-0077
  </div>
  <div style='margin-top:.9rem;font-size:.62rem;color:#2E2B28;'>
